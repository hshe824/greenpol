...
def combine_cofe_h5_pointing(dd,h5pointing,outfile='combined_data.pkl'):
    ...
    tout = np.interp(dd['rev'],prev,prev)
    
    f=open(outfile,'wb')
    combined_data={'sci_data':dd,'az': azout,'el':elout, 'gpstime': tout}
...
def plotnow_aztimesig(fpath,yrmoday,chan,var,st_hour,st_minute,ed_hour,ed_minute,supply_index=False):
    ...
    #x, y = np.linspace(0., 360., steps), np.linspace(, 90., steps)
    
    #az, t = np.meshgrid(x, y)
    az, t = np.meshgrid(az1, t1)

    z = np.zeros(steps**2)
    sig = np.reshape(z, (len(t1), len(az1)))
    ...
    '''
    it = np.where(abs(t - t1) < epsilon)[0]
    iaz = np.where(abs(az.T - az1) < epsilon)[0]
